...
`OTEL_PROPAGATORS` is accepted as a comma-separated list, but only `tracecontext` is implemented. Any other propagator in the list, including `baggage`, is ignored with a warning. W3C Baggage is currently not supported at all: the `BAGGAGE` environment variable is neither read nor written, and baggage is not propagated over HTTP. This is deliberate rather than merely unimplemented. Baggage is arbitrary application-controlled data, and this project propagates context outward very aggressively, into child processes, into containers started by instrumented `docker` commands, across `sudo`, and into `$GITHUB_ENV`. Propagating baggage along all of those paths by default would spread user-controlled values considerably further than most users would expect, which is the exposure the specification warns about for environment variable carriers. If baggage is supported in the future, it will be read-only pass-through and opt-in.
...
### Shell Functions
By design, shell functions are not automatically instrumented. The aliasing mechanism used for auto-instrumentation only works for external commands, built-ins, and aliases, but cannot intercept function calls within the shell's execution context.

**Workaround:** Use `otel_observe` to manually instrument function calls where observability is needed.

### Absolute and Relative Path Commands
Commands called via absolute or relative paths (e.g., `/bin/cat` or `./mycommand`) are not automatically instrumented by default. The auto-instrumentation mechanism relies on command name resolution through the shell's normal lookup process, which bypasses aliasing for explicit paths.

**Workaround:** Use `otel_observe` to manually instrument these commands, or enable `OTEL_SHELL_CONFIG_INSTRUMENT_ABSOLUTE_PATHS=TRUE` (experimental) to instrument them automatically.
