...
def guess_cloud_resource_detectors():
    if (
        ...
        and os.environ.get("RUNNER_ENVIRONMENT", "unknown") == "github-hosted"
    ):
    ...
    ) or file_contains("/sys/hypervisor/uuid", "ec2"):
...
def handle(scope, version, command, arguments):
    ...
    if command.startswith("SPAN_") and not initialized_traces:
        ...
        sampling_strategy = os.environ.get(
        ...
        if traces_exporters:
            ...
            class MyIdGenerator(RandomIdGenerator):
                ...
                def __init__(self):
                    traceparent = os.environ.get(
                        "OTEL_ID_GENERATOR_OVERRIDE_TRACEPARENT", None
                    )
                    if traceparent:
            ...
            tracer_provider = TracerProvider(
                sampler=sampler, resource=final_resources, id_generator=MyIdGenerator()
            )
    ...
    if command.startswith("COUNTER_") and not initialized_metrics:
        ...
        if metrics_exporters:
            ...
            set_meter_provider(
                MeterProvider(metric_readers=metric_readers, resource=final_resources)
            )
    ...
    if command == "RESOURCE_ATTRIBUTE":
        ...
        final_resources = (
            get_aggregated_resources(
                guess_cloud_resource_detectors()
                + [
        ...
        span = get_tracer(scope, version).start_span(
            name,
        ...
        if type == "counter":
            counters[counter_id] = meter.create_counter(
                name, unit=unit, description=description
            )
        elif type == "up_down_counter":
            counters[counter_id] = meter.create_up_down_counter(
                name, unit=unit, description=description
            )
        elif type == "gauge":
            counters[counter_id] = meter.create_gauge(
                name, unit=unit, description=description
            )
        elif type == "histogram":
            counters[counter_id] = meter.create_histogram(
                name,
                unit=unit,
            ...
            counters[counter_id] = meter.create_observable_counter(
                name,
                [functools.partial(observable_counter_callback, counter_id)],
                unit=unit,
                description=description,
            )
            ...
            delayed_observations[counter_id] = {}
            counters[counter_id] = meter.create_observable_up_down_counter(
                name,
                [functools.partial(observable_counter_callback, counter_id)],
                unit=unit,
                description=description,
            ...
            delayed_observations[counter_id] = {}
            counters[counter_id] = meter.create_observable_gauge(
                name,
                [functools.partial(observable_counter_callback, counter_id)],
                unit=unit,
                description=description,
        ...
        if hasattr(counter, "record"):
            ...
            delayed_observations[counter_id][
                hashlib.sha256(
                    json.dumps(observation["attributes"], sort_keys=True).encode(
                        "utf-8"
                    )
                ).hexdigest()
            ] = observation
        ...
        observations[observation_id] = {
            "amount": convert_type("auto", amount),
            "attributes": {},
        }
        with open(response_path, "w") as response:
...
def parse_time(time_string):
    if time_string == "auto":
        ...
        return int(
            datetime.strptime(time_part, "%Y-%m-%dT%H:%M:%S")
            .replace(tzinfo=timezone.utc)
            .timestamp()
            * int(1e9)
        ) + int(fractional_seconds_part.ljust(9, "0")[:9])
    elif "." in time_string:
        seconds_part, fractional_seconds_part = time_string.split(".")
        return int(seconds_part) * int(1e9) + int(
            fractional_seconds_part.ljust(9, "0")[:9]
        )
    else:
