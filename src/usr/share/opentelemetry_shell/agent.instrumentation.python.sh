...
_otel_inject_python() {
  ...
  if _otel_string_starts_with "$version" 3.; then
    ...
    if _otel_can_inject_python_otel && \[ -d /usr/share/opentelemetry_shell/agent.instrumentation.python/"$version"/site-packages/wrapt ]; then
...
" "${command#\\}" "$@"
    ...
    if \[ "${my_code_source:-}" = stdin ]; then
      {
        \cat /usr/share/opentelemetry_shell/agent.instrumentation.python/deep.py
        \cat
