...
if \[ -p "$_otel_remote_sdk_pipe" ]; then
  ...
  otel_init() {
    ...
    if \[ -n "${OTEL_SHELL_SDK_FACTORY_PIPE:-}" ] && \[ -w "$OTEL_SHELL_SDK_FACTORY_PIPE" ] && \[ "${OTEL_LOGS_EXPORTER:-otlp}" != console ] && \[ "${OTEL_METRICS_EXPORTER:-otlp}" != console ] && \[ "${OTEL_TRACES_EXPORTER:-otlp}" != console ]; then
      \echo shell "$(_otel_package_version opentelemetry-shell)" "$_otel_remote_sdk_pipe" >> "$OTEL_SHELL_SDK_FACTORY_PIPE"
    else
