...
_otel_inject_shell_args_with_copy() {
  ...
  while \[ "$#" -gt 0 ]; do
    if \[ "${no_more_options:-0}" != 1 ] && _otel_string_starts_with "$1" -; then
      case "$1" in
        ...
        -*c*)
          local is_script=0
          _otel_escape_arg "$(\printf '%s' "$1" | \sed 's/c//g')"
          \echo -n " "
          ;;
        *)
          _otel_escape_arg "$1"
  ...
  \echo -n " "
  if \[ "$is_script" -eq 1 ]; then _otel_escape_arg "$command"; elif \[ "$#" -gt 0 ]; then
    _otel_escape_arg "$1"
    shift
  else _otel_escape_arg "$shell"; fi
  ...
  if ! \[ -f "$temporary_script" ] && ! \[ -t 0 ]; then
    {
      \echo ". otel.sh"
      \cat
    } | \eval OTEL_SHELL_IS_DYNAMIC=TRUE OTEL_SHELL_COMMANDLINE_OVERRIDE=\""$(_otel_escape_arg "$cmdline")"\" OTEL_SHELL_COMMANDLINE_OVERRIDE_SIGNATURE=$$ OTEL_SHELL_AUTO_INJECTED=TRUE _otel_call "$injected_command_string" || local exit_code=$?
  else
