...
_otel_inject_java() {
  local command="$1"
  shift
  local version="$(_otel_call "$command" --version | \head -n 1 | \cut -d ' ' -f 2 | \cut -d . -f 1)"
