...
_otel_inject_xargs() {
  local args_length=$(($(\getconf ARG_MAX) - $(\env | \wc -c) - $(\env | \egrep'^[^ ]+=' | wc -l) * 4 - $(\echo "$*" | \wc -c) - 2048)) # this is an estimation of the default according to https://www.in-ulm.de/~mascheck/various/argmax/
