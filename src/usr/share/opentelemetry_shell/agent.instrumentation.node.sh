...
_otel_inject_node_args() {
  ...
  if \[ -d "$_otel_shell_home"/agent.instrumentation.node/"$version" ]; then
    \echo -n ' '; _otel_escape_args --require "$_otel_shell_home"/agent.instrumentation.node/"$version"/deep.inject.js
  fi
  while \[ "$#" -gt 0 ]; do
    \echo -n ' '
    if \[ "$1" = -e ] || \[ "$1" = --eval ] || \[ "$1" = -p ] || \[ "$1" = --print ]; then
      ...
      if \[ "${OTEL_SHELL_CONFIG_INJECT_DEEP:-FALSE}" = TRUE ] && \[ -d "$_otel_shell_home"/agent.instrumentation.node/"$version" ]; then
        ...
        if _otel_is_node_injected "$dir"; then
          _otel_escape_args --require "$_otel_shell_home"/agent.instrumentation.node/"$version"/deep.link.js
        elif \[ -z "${OTEL_TRACES_EXPORTER:-}" ] || \[ "${OTEL_TRACES_EXPORTER:-}" = console ] || \[ "${OTEL_TRACES_EXPORTER:-}" = otlp ]; then
          _otel_escape_args --require "$_otel_shell_home"/agent.instrumentation.node/"$version"/deep.instrument.js
        fi
