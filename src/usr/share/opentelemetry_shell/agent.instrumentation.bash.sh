...
_otel_inject_shell_args_with_c_flag() {
  ...
  # arguments
  if \[ -n "$dollar_zero" ]; then
    _otel_escape_arg "$dollar_zero"
    \echo -n " "
  fi
  for dollar_n in "$@"; do
    _otel_escape_arg "$dollar_n"
    \echo -n " "
  done
}
  ...
  if \[ "$#" -eq 1 ] && ! [ -t 0 ]; then
    {
      \echo ". otel.sh"
      \cat
    } | OTEL_SHELL_IS_DYNAMIC=TRUE OTEL_SHELL_COMMANDLINE_OVERRIDE="$cmdline" OTEL_SHELL_COMMANDLINE_OVERRIDE_SIGNATURE="$$" OTEL_SHELL_AUTO_INJECTED=TRUE \eval _otel_call "$injected_command_string" || local exit_code=$?
  else
