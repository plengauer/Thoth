...
import { LLMProvider, LLMSystem, MimeType, OpenInferenceSpanKind, SemanticConventions, } from "@arizeai/openinference-semantic-conventions";
import { consumeResponseStreamEvents, getResponsesInputMessagesAttributes, getResponsesOutputMessagesAttributes, getResponsesUsageAttributes, } from "./responsesAttributes.js";
import { assertUnreachable, isString } from "./typeUtils.js";
 ...
 */
export const HOST_SUFFIX_TO_PROVIDER = {
...
export class OpenAIInstrumentation extends InstrumentationBase {
    ...
    init() {
        const module = new InstrumentationNodeModuleDefinition("openai", 
        // 5.x is best effort
    ...
    if (typeof message.content === "string") {
        ...
        message.content.forEach((part, index) => {
            ...
            if (part.type === "text") {
                ...
                attributes[`${contentsIndexPrefix}${SemanticConventions.MESSAGE_CONTENT_IMAGE}.${SemanticConventions.IMAGE_URL}`] = part.image_url.url;
            }
        });
    }
    switch (role) {
            ...
            if (message.tool_calls) {
                message.tool_calls.forEach((toolCall, index) => {
                    ...
                    if (toolCall.type === "function") {
                        ...
                        attributes[`${toolCallIndexPrefix}${SemanticConventions.TOOL_CALL_FUNCTION_ARGUMENTS_JSON}`] = toolCall.function.arguments;
                    }
                    else {
        ...
        case "tool":
            if (message.tool_call_id) {
                attributes[`${SemanticConventions.MESSAGE_TOOL_CALL_ID}`] = message.tool_call_id;
            }
            break;
        case "system":
...
function getUsageAttributes(completion) {
    if (completion.usage) {
        const usageAttributes = {
            ...
            [SemanticConventions.LLM_TOKEN_COUNT_PROMPT_DETAILS_CACHE_READ]: completion.usage.prompt_tokens_details?.cached_tokens,
            [SemanticConventions.LLM_TOKEN_COUNT_PROMPT_DETAILS_AUDIO]: completion.usage.prompt_tokens_details?.audio_tokens,
            [SemanticConventions.LLM_TOKEN_COUNT_COMPLETION_DETAILS_AUDIO]: completion.usage.completion_tokens_details?.audio_tokens,
            [SemanticConventions.LLM_TOKEN_COUNT_COMPLETION_DETAILS_REASONING]: completion.usage.completion_tokens_details?.reasoning_tokens,
        };
 ...
 */
function getChatCompletionFinishReasonAttributes(chatCompletion) {
    const choice = chatCompletion.choices[0];
...
function getChatCompletionOutputMessageAttributes(message) {
    ...
    if (message.function_call) {
        attributes[SemanticConventions.MESSAGE_FUNCTION_CALL_NAME] = message.function_call.name;
...
function getToolAndFunctionCallAttributesFromStreamChunk(chunk) {
    ...
    if (choice.delta.function_call) {
        attributes[SemanticConventions.MESSAGE_FUNCTION_CALL_NAME] = choice.delta.function_call.name;
        attributes[SemanticConventions.MESSAGE_FUNCTION_CALL_ARGUMENTS_JSON] =
