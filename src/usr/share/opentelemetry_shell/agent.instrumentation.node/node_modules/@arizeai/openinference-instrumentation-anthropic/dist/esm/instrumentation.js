...
export class AnthropicInstrumentation extends InstrumentationBase {
    ...
    init() {
        const module = new InstrumentationNodeModuleDefinition("@anthropic-ai/sdk", ["*"], // Try accepting any version
    ...
    patch(module, moduleVersion) {
        ...
        const anthropicModule = module.default || module;
        ...
        (original) => {
            return function patchedCreate(...args) {
                ...
                const wrappedPromise = execPromise.then(wrappedPromiseThen).catch((error) => {
                    span.recordException(error);
                    span.setStatus({
                        code: SpanStatusCode.ERROR,
                        message: error.message,
            ...
            if (part.type === "text") {
                ...
                if (part.source.type === "base64") {
                    ...
                    attributes[`${contentsIndexPrefix}${SemanticConventions.MESSAGE_CONTENT_IMAGE}.type`] =
                        "base64";
                    attributes[`${contentsIndexPrefix}${SemanticConventions.MESSAGE_CONTENT_IMAGE}.media_type`] = part.source.media_type;
                }
            }
                ...
                attributes[`${toolCallIndexPrefix}${SemanticConventions.TOOL_CALL_FUNCTION_NAME}`] =
                    part.name;
                attributes[`${toolCallIndexPrefix}${SemanticConventions.TOOL_CALL_FUNCTION_ARGUMENTS_JSON}`] = JSON.stringify(part.input);
            }
...
function getAnthropicUsageAttributes(message) {
    if (message.usage) {
        return {
            ...
            [SemanticConventions.LLM_TOKEN_COUNT_TOTAL]: message.usage.input_tokens + message.usage.output_tokens,
        };
    }
    return {};
}
 ...
 */
async function consumeAnthropicStreamChunks(stream, span) {
    let streamResponse = "";
