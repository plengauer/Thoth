...
import { isTextContent, isToolUseContent } from "../types/bedrock-types.js";
import { setSpanAttribute } from "./attribute-helpers.js";
import { isSimpleTextResponse, normalizeResponseContentBlocks, normalizeUsageAttributes, parseResponseBody, } from "./invoke-model-helpers.js";
/**
 ...
 */
function addOutputMessageContentAttributes({ message, span, }) {
    if (!Array.isArray(message.content)) {
        return;
    }
    // Set the message role
    setSpanAttribute(span, `${SemanticConventions.LLM_OUTPUT_MESSAGES}.0.${SemanticConventions.MESSAGE_ROLE}`, message.role);
...
const extractOutputMessagesAttributes = withSafety({
    fn: ({ normalizedMessage, span }) => {
        setSpanAttribute(span, `${SemanticConventions.LLM_OUTPUT_MESSAGES}.0.${SemanticConventions.MESSAGE_ROLE}`, normalizedMessage.role);
        ...
        if (isSimpleTextResponse(normalizedMessage)) {
            ...
            setSpanAttribute(span, `${SemanticConventions.LLM_OUTPUT_MESSAGES}.0.${SemanticConventions.MESSAGE_CONTENT}`, textBlock.text);
        }
        else {
 ...
 * @param params.span The OpenTelemetry span to set attributes on
 */
function extractUsageAttributes({ responseBody, modelType, span, }) {
...
//# sourceMappingURL=invoke-model-response-attributes.js.map