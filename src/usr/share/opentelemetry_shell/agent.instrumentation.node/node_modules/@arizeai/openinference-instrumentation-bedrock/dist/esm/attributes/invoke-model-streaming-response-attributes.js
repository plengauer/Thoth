...
import { setSpanAttribute } from "./attribute-helpers.js";
import { normalizeUsageAttributes } from "./invoke-model-helpers.js";
/**
 * Type guard to check if an unknown object is a valid stream chunk
 */
function isValidStreamChunk(chunk) {
    return (isObjectWithStringKeys(chunk) &&
...
function setStreamingOutputAttributes({ span, outputText, contentBlocks, usage, }) {
    ...
    toolUseBlocks.forEach((block, toolCallIndex) => {
        ...
        setSpanAttribute(span, `${toolCallPrefix}.${SemanticConventions.TOOL_CALL_ID}`, block.id || "unknown");
        setSpanAttribute(span, `${toolCallPrefix}.${SemanticConventions.TOOL_CALL_FUNCTION_NAME}`, block.name || "unknown");
        setSpanAttribute(span, `${toolCallPrefix}.${SemanticConventions.TOOL_CALL_FUNCTION_ARGUMENTS_JSON}`, JSON.stringify(block.input || {}));
    });
    ...
    if (usage.cache_read_input_tokens !== undefined) {
        setSpanAttribute(span, SemanticConventions.LLM_TOKEN_COUNT_PROMPT_DETAILS_CACHE_READ, usage.cache_read_input_tokens);
    }
    if (usage.cache_creation_input_tokens !== undefined) {
        setSpanAttribute(span, SemanticConventions.LLM_TOKEN_COUNT_PROMPT_DETAILS_CACHE_WRITE, usage.cache_creation_input_tokens);
    }
}
/**
 ...
 */
export function safelySplitStream({ originalStream }) {
    try {
...
export const consumeBedrockStreamChunks = withSafety({
    fn: async ({ stream, span, modelType, }) => {
