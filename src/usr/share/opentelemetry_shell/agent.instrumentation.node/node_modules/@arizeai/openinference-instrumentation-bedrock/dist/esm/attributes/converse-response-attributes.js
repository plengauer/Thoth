...
import { isConverseToolUseContent } from "../types/bedrock-types.js";
import { processMessages, setSpanAttribute } from "./attribute-helpers.js";
/**
 ...
 */
function extractBaseResponseAttributes({ span, response, }) {
 ...
 */
function extractOutputMessagesAttributes({ span, response, }) {
 ...
 */
function extractToolCallAttributes({ span, response, }) {
    const output = response.output;
    if (!output || !("message" in output))
        return;
    const outputMessage = output.message;
    ...
    toolUseBlocks.forEach((content, toolCallIndex) => {
        ...
        if (toolUse) {
            setSpanAttribute(span, `${SemanticConventions.LLM_OUTPUT_MESSAGES}.0.${SemanticConventions.MESSAGE_TOOL_CALLS}.${toolCallIndex}.${SemanticConventions.TOOL_CALL_FUNCTION_NAME}`, toolUse.name);
            setSpanAttribute(span, `${SemanticConventions.LLM_OUTPUT_MESSAGES}.0.${SemanticConventions.MESSAGE_TOOL_CALLS}.${toolCallIndex}.${SemanticConventions.TOOL_CALL_FUNCTION_ARGUMENTS_JSON}`, JSON.stringify(toolUse.input));
            setSpanAttribute(span, `${SemanticConventions.LLM_OUTPUT_MESSAGES}.0.${SemanticConventions.MESSAGE_TOOL_CALLS}.${toolCallIndex}.${SemanticConventions.TOOL_CALL_ID}`, toolUse.toolUseId);
        }
    });
}
/**
 ...
 */
function extractUsageAttributes({ span, response, }) {
...
//# sourceMappingURL=converse-response-attributes.js.map