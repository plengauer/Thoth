...
import { LLMSystem, MimeType, SemanticConventions, } from "@arizeai/openinference-semantic-conventions";
import { isImageContent, isTextContent, isToolUseContent } from "../types/bedrock-types.js";
import { extractModelName, setSpanAttribute } from "./attribute-helpers.js";
import { extractInvocationParameters, extractToolResultBlocks, formatImageUrl, normalizeRequestContentBlocks, parseRequestBody, } from "./invoke-model-helpers.js";
// Helper functions
/**
 ...
 */
function addMessageAttributes({ span, message, index, }) {
    if (!message.role)
        return;
    setSpanAttribute(span, `${SemanticConventions.LLM_INPUT_MESSAGES}.${index}.${SemanticConventions.MESSAGE_ROLE}`, message.role);
    if (typeof message.content === "string") {
        setSpanAttribute(span, `${SemanticConventions.LLM_INPUT_MESSAGES}.${index}.${SemanticConventions.MESSAGE_CONTENT}`, message.content);
    }
    else {
 ...
 */
function handleToolCallsInMessage({ span, message, messageIndex, }) {
    if (!Array.isArray(message.content))
        return;
    let toolCallIndex = 0;
    message.content.forEach((content) => {
        if (isToolUseContent(content)) {
            ...
            setSpanAttribute(span, `${toolCallPrefix}.${SemanticConventions.TOOL_CALL_FUNCTION_NAME}`, content.name);
            setSpanAttribute(span, `${toolCallPrefix}.${SemanticConventions.TOOL_CALL_FUNCTION_ARGUMENTS_JSON}`, JSON.stringify(content.input));
            toolCallIndex++;
        }
    });
}
 ...
 * @param params.messageIndex The message index in the conversation
 */
function handleToolResultsInMessage({ span, message, messageIndex, }) {
    if (!Array.isArray(message.content))
        return;
    const toolResultBlocks = extractToolResultBlocks(message.content);
    toolResultBlocks.forEach((contentBlock) => {
        setSpanAttribute(span, `${SemanticConventions.LLM_INPUT_MESSAGES}.${messageIndex}.${SemanticConventions.MESSAGE_TOOL_CALL_ID}`, contentBlock.tool_use_id);
    });
}
/**
 ...
 * @param params.messageIndex The message index in the conversation
 */
function addMessageContentAttributes({ span, message, messageIndex, }) {
    if (!message.content)
        return;
    if (Array.isArray(message.content)) {
 ...
 */
function extractBaseRequestAttributes({ span, command, requestBody, system, }) {
 ...
 */
function extractInputMessagesAttributes({ span, requestBody, system, }) {
 ...
 */
function extractInputToolAttributes({ span, requestBody, system, }) {
    if (system === LLMSystem.AMAZON &&
        ...
        requestBody.toolConfig.tools.forEach((tool, index) => {
            ...
            if (isObjectWithStringKeys(tool)) {
                setSpanAttribute(span, `${SemanticConventions.LLM_TOOLS}.${index}.${SemanticConventions.TOOL_JSON_SCHEMA}`, JSON.stringify(tool));
            }
        });
    }
...
//# sourceMappingURL=invoke-model-request-attributes.js.map