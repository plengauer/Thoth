...
exports.normalizeUsageAttributes = exports.normalizeResponseContentBlocks = exports.parseResponseBody = exports.normalizeRequestContentBlocks = exports.parseRequestBody = void 0;
exports.isSimpleTextResponse = isSimpleTextResponse;
exports.formatImageUrl = formatImageUrl;
exports.extractInvocationParameters = extractInvocationParameters;
exports.extractToolResultBlocks = extractToolResultBlocks;
exports.coerceNovaToMessageContent = coerceNovaToMessageContent;
exports.convertMetaToMessageContent = convertMetaToMessageContent;
 ...
 */
function isSimpleTextResponse(message) {
    return Boolean(Array.isArray(message.content) &&
        message.content.length === 1 &&
        (0, bedrock_types_1.isTextContent)(message.content[0]));
}
/**
 ...
 */
function formatImageUrl(source) {
 ...
 */
exports.parseRequestBody = (0, openinference_core_1.withSafety)({
    fn: (command) => {
        var _a;
        if (!((_a = command.input) === null || _a === void 0 ? void 0 : _a.body)) {
    ...
    onError: (error) => {
        api_1.diag.warn("Error parsing InvokeModel request body:", error);
        return null;
    },
});
/**
 ...
 */
function extractInvocationParameters(requestBody, system) {
    if (system === openinference_semantic_conventions_1.LLMSystem.AMAZON &&
    ...
    else if (system === openinference_semantic_conventions_1.LLMSystem.AMAZON &&
        ...
        const { system, messages, tools, prompt } = requestBody, invocationParams = __rest(requestBody, ["system", "messages", "tools", "prompt"]);
...
function extractToolResultBlocks(content) {
    ...
    return content.filter(bedrock_types_1.isToolResultContent);
}
/**
 ...
 */
function isNovaRequest(requestBody) {
    return ("messages" in requestBody &&
 ...
 */
function isTitanRequest(requestBody) {
 ...
 */
function convertSimpleTextToBedrockMessages(requestBody, textFieldName) {
 ...
 */
function convertNovaToBedrockMessages(requestBody) {
 ...
 */
function isMistralTextCompletionRequest(requestBody) {
 ...
 */
function isMistralChatRequest(requestBody) {
    return ("messages" in requestBody &&
        Array.isArray(requestBody.messages) &&
        requestBody.messages.length > 0);
}
 ...
 */
function convertMistralChatToBedrockMessages(requestBody) {
    const messages = requestBody.messages;
    return messages.map((message) => {
        var _a;
        ...
        if (message.role === "assistant" && message.tool_calls) {
            const content = message.tool_calls.map((toolCall) => ({
                type: "tool_use",
                id: toolCall.id,
        ...
        if (Array.isArray(message.content)) {
            ...
            for (const contentBlock of message.content) {
                ...
                else if (contentBlock.type === "image_url" && ((_a = contentBlock.image_url) === null || _a === void 0 ? void 0 : _a.url)) {
 ...
 * @returns {BedrockMessage[]} Array of converted BedrockMessage objects
 */
function convertAI21JambaToBedrockMessages(requestBody) {
    const messages = requestBody.messages;
    return messages.map((message) => ({
        role: message.role,
        content: [
 ...
 */
function fallbackNormalizeRequestContentBlocks(requestBody) {
 ...
 */
exports.normalizeRequestContentBlocks = (0, openinference_core_1.withSafety)({
    fn: (requestBody, llm_system) => {
        let messages = [];
        if (llm_system === openinference_semantic_conventions_1.LLMSystem.ANTHROPIC) {
            messages = requestBody.messages;
        }
        else if (llm_system === openinference_semantic_conventions_1.LLMSystem.AMAZON) {
        ...
        else if (llm_system === openinference_semantic_conventions_1.LLMSystem.COHERE &&
        ...
        else if (llm_system === openinference_semantic_conventions_1.LLMSystem.META &&
            "prompt" in requestBody &&
            typeof requestBody.prompt === "string") {
            // Handle Meta format: { prompt: string }
            messages = convertSimpleTextToBedrockMessages(requestBody, "prompt");
        }
        else if (llm_system === openinference_semantic_conventions_1.LLMSystem.MISTRALAI) {
        ...
        else if (llm_system === openinference_semantic_conventions_1.LLMSystem.AI21 &&
            "prompt" in requestBody &&
            typeof requestBody.prompt === "string") {
    ...
    onError: (error) => {
        api_1.diag.warn("Error normalizing request content blocks:", error);
        return [];
    },
});
 ...
 */
exports.parseResponseBody = (0, openinference_core_1.withSafety)({
    fn: (response) => {
        if (!response.body) {
    ...
    onError: (error) => {
        api_1.diag.warn("Error parsing response body:", error);
        return null;
    },
});
/**
 ...
 */
function coerceNovaToMessageContent(content) {
 ...
 */
function extractNovaContent(responseBody) {
 ...
 */
function isNovaResponse(responseBody) {
 ...
 */
function isTitanResponse(responseBody) {
 ...
 */
function convertAI21JambaToMessageContent(responseBody) {
    var _a, _b;
            ...
            if (message) {
                ...
                if (Array.isArray(message.tool_calls)) {
                    ...
                    for (const toolCall of toolCalls) {
                        if (((_a = toolCall === null || toolCall === void 0 ? void 0 : toolCall.function) === null || _a === void 0 ? void 0 : _a.name) && ((_b = toolCall === null || toolCall === void 0 ? void 0 : toolCall.function) === null || _b === void 0 ? void 0 : _b.arguments)) {
                            try {
                                content.push({
                                    type: "tool_use",
                                ...
                                api_1.diag.warn("Failed to parse AI21 tool call arguments:", error);
                            }
                        }
                    }
                }
 ...
 */
function convertMetaToMessageContent(responseBody) {
 ...
 */
function convertArrayFieldToMessageContent(responseBody, arrayFieldName, textFieldName) {
 ...
 */
exports.normalizeResponseContentBlocks = (0, openinference_core_1.withSafety)({
    fn: (responseBody, llm_system) => {
    ...
    onError: (error) => {
        api_1.diag.warn("Error normalizing content blocks:", error);
        return {
            role: "assistant",
            content: [],
        };
 ...
 */
exports.normalizeUsageAttributes = (0, openinference_core_1.withSafety)({
    fn: (responseBody, llm_system) => {
        var _a;
        if (llm_system === openinference_semantic_conventions_1.LLMSystem.ANTHROPIC) {
            ...
            if (isNovaResponse(responseBody)) {
                ...
                return {
                    ...
                    cache_read_input_tokens: typeof usage.cacheReadInputTokenCount === "number"
                        ? usage.cacheReadInputTokenCount
                        : undefined,
                    cache_creation_input_tokens: typeof usage.cacheWriteInputTokenCount === "number"
                ...
                const inputTokens = typeof responseBody.inputTextTokenCount === "number"
                    ? responseBody.inputTextTokenCount
                    : undefined;
                const results = responseBody.results;
                const outputTokens = typeof ((_a = results === null || results === void 0 ? void 0 : results[0]) === null || _a === void 0 ? void 0 : _a.tokenCount) === "number" ? results[0].tokenCount : undefined;
                const result = {};
            ...
            return {
                input_tokens: typeof responseBody.prompt_token_count === "number"
                    ? responseBody.prompt_token_count
                    : undefined,
                output_tokens: typeof responseBody.generation_token_count === "number"
    ...
    onError: (error) => {
        api_1.diag.warn("Error normalizing usage attributes:", error);
        return {};
    },
});
//# sourceMappingURL=invoke-model-helpers.js.map