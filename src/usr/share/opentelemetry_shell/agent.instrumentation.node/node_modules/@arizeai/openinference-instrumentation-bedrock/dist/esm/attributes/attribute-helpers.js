...
import { LLMProvider, LLMSystem, OpenInferenceSpanKind, SemanticConventions, } from "@arizeai/openinference-semantic-conventions";
import { isConverseImageContent, isConverseTextContent, isConverseToolResultContent, isConverseToolUseContent, } from "../types/bedrock-types.js";
import { formatImageUrl } from "./invoke-model-helpers.js";
/**
 ...
 */
export function getSystemFromModelId(modelId) {
    if (modelId.includes("anthropic"))
        return LLMSystem.ANTHROPIC;
    if (modelId.includes("ai21"))
        return LLMSystem.AI21;
    if (modelId.includes("amazon"))
        return LLMSystem.AMAZON;
    if (modelId.includes("cohere"))
        return LLMSystem.COHERE;
    if (modelId.includes("meta"))
        return LLMSystem.META;
    if (modelId.includes("mistral"))
        return LLMSystem.MISTRALAI;
    return LLMSystem.AMAZON;
}
export function setBasicSpanAttributes(span, llm_system) {
 ...
 */
export function aggregateSystemPrompts(systemPrompts) {
 ...
 */
export function aggregateMessages(systemPrompts = [], messages = []) {
    const aggregated = [];
 ...
 */
export function getAttributesFromMessageContent(content) {
    const attributes = {};
 ...
 */
export function getAttributesFromMessage(message) {
    const attributes = {};
    if (message.role) {
 ...
 */
export function processMessages({ span, messages, baseKey, }) {
