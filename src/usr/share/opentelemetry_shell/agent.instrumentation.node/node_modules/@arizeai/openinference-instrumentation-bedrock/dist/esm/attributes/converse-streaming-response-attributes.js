...
import { assertUnreachable, isObjectWithStringKeys, safelyJSONParse, safelyJSONStringify, withSafety, } from "@arizeai/openinference-core";
import { MimeType, SemanticConventions } from "@arizeai/openinference-semantic-conventions";
import { isValidConverseStreamEventData, toNormalizedConverseStreamEvent, } from "../types/bedrock-types.js";
...
function startToolCall(state, { id, name, contentBlockIndex }) {
    if (contentBlockIndex !== undefined) {
        state.toolUseIdByIndex ?? (state.toolUseIdByIndex = {});
...
function setConverseStreamingOutputAttributes({ span, outputText, toolCalls, usage, stopReason, }) {
    ...
    toolCalls.forEach((toolCall, toolCallIndex) => {
        ...
        setSpanAttribute(span, `${toolCallPrefix}.${SemanticConventions.TOOL_CALL_FUNCTION_NAME}`, toolCall.name);
        setSpanAttribute(span, `${toolCallPrefix}.${SemanticConventions.TOOL_CALL_FUNCTION_ARGUMENTS_JSON}`, safelyJSONStringify(toolCall.input));
    });
