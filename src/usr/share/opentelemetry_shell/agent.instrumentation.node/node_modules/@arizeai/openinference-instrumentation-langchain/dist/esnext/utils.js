...
export const SESSION_ID_KEYS = ["session_id", "thread_id", "conversation_id"];
/**
 * Handler for any unexpected errors that occur during processing.
 */
const onError = (message) => (error) => {
    diag.warn(`OpenInference-LangChain: error processing langchain run, falling back to null. ${message}. ${error}`);
};
...
function parseRetrievalDocument(document) {
    ...
    if (isObject(document.metadata)) {
        parsedDocument["document.metadata"] = (_a = safelyJSONStringify(document.metadata)) !== null && _a !== void 0 ? _a : undefined;
    }
...
function formatPromptTemplate(run) {
    ...
    return {
        [SemanticConventions.PROMPT_TEMPLATE_VARIABLES]: (_a = safelyJSONStringify(run.inputs)) !== null && _a !== void 0 ? _a : undefined,
        [SemanticConventions.PROMPT_TEMPLATE_TEMPLATE]: (_b = safelyGetTemplateFromSerialized(run.serialized)) !== null && _b !== void 0 ? _b : undefined,
    };
}
...
function formatTokenCounts(outputs) {
    ...
    if (isObject(llmOutput.tokenUsage)) {
        return {
            [SemanticConventions.LLM_TOKEN_COUNT_COMPLETION]: getTokenCount(llmOutput.tokenUsage.completionTokens),
            [SemanticConventions.LLM_TOKEN_COUNT_PROMPT]: getTokenCount(llmOutput.tokenUsage.promptTokens),
    ...
    if (isObject(llmOutput.estimatedTokenUsage)) {
        return {
            [SemanticConventions.LLM_TOKEN_COUNT_COMPLETION]: getTokenCount(llmOutput.estimatedTokenUsage.completionTokens),
            [SemanticConventions.LLM_TOKEN_COUNT_PROMPT]: getTokenCount(llmOutput.estimatedTokenUsage.promptTokens),
            [SemanticConventions.LLM_TOKEN_COUNT_TOTAL]: getTokenCount(llmOutput.estimatedTokenUsage.totalTokens),
        };
    }
    /**
