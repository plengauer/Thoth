 ...
 *  }) // returns true so the rule applies and the value will be redacted
 */
const maskPromptsRule = {
    condition: ({ config, key }) => config.hidePrompts && key === openinference_semantic_conventions_1.SemanticConventions.LLM_PROMPTS,
    action: () => constants_1.REDACTED_VALUE,
};
/**
 ...
 * ```
 */
const maskLLMToolsRule = {
    condition: ({ config, key }) => (config.hideInputs || config.hideLLMTools) && key.includes(openinference_semantic_conventions_1.SemanticConventions.LLM_TOOLS),
    action: () => undefined,
};
/**
