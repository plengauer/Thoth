...
if ! [ -r "$package" ]; then
  ...
  curl -v --no-progress-meter https://github.com/plengauer/Thoth/releases/latest 2>&1 | grep location | tr -d '\r' | while IFS= read -r line; do echo "${line##*/v}"; done | case "$extension" in
...
if ! [ -r "$package" ]; then
  ...
  curl -v --no-progress-meter https://github.com/plengauer/Thoth/releases/latest 2>&1 | grep location | tr -d '\r' | while IFS= read -r line; do echo "${line##*/v}"; done | xargs -I '{}' echo -n opentelemetry-shell_'{}'."$extension" | xargs -r -I '{}' wget -O "$package" https://github.com/plengauer/Thoth/releases/latest/download/'{}'
fi
if ! [ -r "$package" ]; then
