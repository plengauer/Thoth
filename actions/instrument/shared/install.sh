...
if ! type otel.sh 2>/dev/null; then
  ...
  if [ "$(uname -s)" = "Darwin" ]; then
    ...
    brew install opentelemetry-shell || (
      ...
      [ "$action_tag_name" = main ] || action_tag_name=v"$version"
      tarball_file=/tmp/opentelemetry-shell.tar.gz
      GITHUB_REPOSITORY="$GITHUB_ACTION_REPOSITORY" gh_release "$action_tag_name" | jq '.assets[] | select(.name | endswith(".tar.gz")) | .url' -r | head -n 1 | xargs -r -I '{}' wget --header "Authorization: ******" --header 'Accept: application/octet-stream' -O "$tarball_file" '{}'
      sudo mkdir -p /usr/local/share /usr/local/bin /usr/local/opt
      sudo tar xzf "$tarball_file" -C /usr/local
      arch_name="$(uname -m)"
      if [ -r /usr/local/usr/share/opentelemetry_shell/agent.instrumentation.http/"$arch_name"/libinjecthttpheader.dylib ]; then
        sudo mv /usr/local/usr/share/opentelemetry_shell/agent.instrumentation.http/"$arch_name"/libinjecthttpheader.dylib /usr/local/usr/share/opentelemetry_shell/agent.instrumentation.http/libinjecthttpheader.dylib
      fi
      sudo rm -rf /usr/local/usr/share/opentelemetry_shell/agent.instrumentation.http/arm64 /usr/local/usr/share/opentelemetry_shell/agent.instrumentation.http/x86_64
      ...
      if type python3 1>/dev/null 2>/dev/null && [ -r /usr/local/opt/opentelemetry_shell/requirements.txt ]; then
        sudo python3 -m venv /usr/local/opt/opentelemetry_shell/venv
        sudo /usr/local/opt/opentelemetry_shell/venv/bin/pip3 install -r /usr/local/opt/opentelemetry_shell/requirements.txt
      fi
      rm "$tarball_file"
    )
  elif [ "$GITHUB_REPOSITORY" = "$GITHUB_ACTION_REPOSITORY" ] && [ -f "$GITHUB_WORKSPACE"/opentelemetry-shell_"$version"_"$arch".deb ]; then
    ...
    debian_file=/var/cache/apt/archives/opentelemetry-shell_"$version"_"$arch".deb
    [ "$action_tag_name" = main ] || action_tag_name=v"$version"
    GITHUB_REPOSITORY="$GITHUB_ACTION_REPOSITORY" gh_release "$action_tag_name" | jq '.assets[] | select(.name | endswith("_'"$arch"'.deb")) | .url' -r | head -n 1 | xargs -r -I '{}' wget --header "Authorization: ******" --header 'Accept: application/octet-stream' -O - '{}' | sudo tee "$debian_file" >/dev/null
    ...
    if [ "$(uname -s)" = "Darwin" ]; then
      curl --fail -L -s -H "Authorization: ******" "${GITHUB_API_URL:-https://api.github.com}"/repos/"$GITHUB_ACTION_REPOSITORY"/releases/latest | jq '.assets[] | select(.name | endswith(".tar.gz")) | .url' -r | head -n 1 | xargs -r -I '{}' wget --header "Authorization: ******" --header 'Accept: application/octet-stream' -O /tmp/package.tar.gz '{}'
      sudo mkdir -p /usr/local/share /usr/local/bin /usr/local/opt
      ...
      curl --fail -L -s -H "Authorization: ******" "${GITHUB_API_URL:-https://api.github.com}"/repos/"$GITHUB_ACTION_REPOSITORY"/releases/latest | jq '.assets[] | select(.name | endswith("_all.deb")) | .url' -r | head -n 1 | xargs -r -I '{}' wget --header "Authorization: ******" --header 'Accept: application/octet-stream' -O /tmp/package.deb '{}'
