...
if ! type otel.sh 2> /dev/null; then
  ...
  if [ -n "$action_tag_name" ]; then
    if [ "$INPUT_CACHE" = "true" ]; then
      ...
      sudo -E -H node -e "require('@actions/cache').restoreCache(['/var/cache/apt/archives/*.deb', '/opt/opentelemetry_shell/venv', '/opt/opentelemetry_shell/collector.image'], '$cache_key');"
    fi
