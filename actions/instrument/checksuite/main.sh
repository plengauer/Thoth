...
check_suite_conclusion="$(jq < "$check_suite_json" -r .conclusion)"

check_suite_span_handle="$(otel_span_start @"$check_suite_started_at" CONSUMER "$check_suite_name")"
otel_span_attribute_typed "$check_suite_span_handle" string github.actions.type=checksuite
otel_span_attribute_typed "$check_suite_span_handle" string github.actions.url="${GITHUB_SERVER_URL:-https://github.com}/$GITHUB_REPOSITORY/actions"
otel_span_attribute_typed "$check_suite_span_handle" int github.checks.suite.id="$(jq < "$check_suite_json" .id)"
otel_span_attribute_typed "$check_suite_span_handle" string github.checks.suite.conclusion="$check_suite_conclusion"
otel_span_attribute_typed "$check_suite_span_handle" string github.checks.suite.head_branch="$(jq < "$check_suite_json" -r .head_branch)"
otel_span_attribute_typed "$check_suite_span_handle" string github.checks.suite.head_sha="$(jq < "$check_suite_json" -r .head_sha)"
otel_span_attribute_typed "$check_suite_span_handle" string github.checks.app.name="$check_suite_name"
otel_span_attribute_typed "$check_suite_span_handle" string github.checks.app.slug="$(jq < "$check_suite_json" -r '.app.slug // empty')"
otel_span_activate "$check_suite_span_handle"
[ -z "${INPUT_DEBUG}" ] || echo "span checksuite $TRACEPARENT $check_suite_name" >&2
if [ "$check_suite_conclusion" = failure ]; then otel_span_error "$check_suite_span_handle"; fi
...
jq -r --unbuffered '. | ["'"$TRACEPARENT"'", .id, .name, .conclusion, .started_at, .completed_at, (.app.name // "unknown"), (.app.slug // "unknown")] | @tsv' < "$filtered_check_runs_json" | sed 's/\t/ /g' | while read -r TRACEPARENT check_run_id check_run_name check_run_conclusion check_run_started_at check_run_completed_at app_name app_slug; do
  ...
  # Metrics observation
  observation_handle="$(otel_observation_create 1)"
  otel_observation_attribute_typed "$observation_handle" string github.checks.run.name="$check_run_name"
  otel_observation_attribute_typed "$observation_handle" string github.checks.run.conclusion="$check_run_conclusion"
  otel_observation_attribute_typed "$observation_handle" string github.checks.app.name="$app_name"
  otel_observation_attribute_typed "$observation_handle" string github.checks.app.slug="$app_slug"
  ...
  # Create span for check run (similar to job span)
  ...
  otel_span_attribute_typed "$check_run_span_handle" int github.checks.run.id="$check_run_id"
  otel_span_attribute_typed "$check_run_span_handle" string github.checks.run.name="$check_run_name"
  otel_span_attribute_typed "$check_run_span_handle" string github.checks.run.conclusion="$check_run_conclusion"
  otel_span_attribute_typed "$check_run_span_handle" string github.checks.app.name="$app_name"
  otel_span_attribute_typed "$check_run_span_handle" string github.checks.app.slug="$app_slug"
