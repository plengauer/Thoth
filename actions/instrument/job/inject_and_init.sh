...
if [ "$GITHUB_JOB" = copilot ] && [ "$GITHUB_WORKFLOW" = 'Copilot coding agent' ]; then
  echo "::notice ::OpenTelemetry for GitHub actions detected Copilot coding agent, which is unable to extract data directly (it may have caused warnings already), reconfiguring for deferred export. This needs the workflow-level instrumentation to forward the data."
