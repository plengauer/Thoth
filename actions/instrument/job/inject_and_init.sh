...
( set +x && printf '%s' "$INPUT_SECRETS_TO_REDACT" | jq -r '. | to_entries[].value' | sed 's/[.[\(*^$+?{|]/\\\\&/g' | grep -v '^$' | xargs -d '\n' -I '{}' echo '::add-mask::{}' >&2 )
mask_patterns="$(printf '%s' "$INPUT_SECRETS_TO_REDACT" | jq -r '. | to_entries[].value' | sed 's/[.[\(*^$+?{|]/\\\\&/g' | grep -v '^$' | sed 's/"/\\"/g')"
