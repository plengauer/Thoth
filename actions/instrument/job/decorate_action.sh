...
if ( type lsof && lsof -p "$redirect_github_logs_pid" -ad 0 -O -b -t 2> /dev/null | \grep -qF -- "$redirect_github_logs_pid" ) || ( kill -0 "$redirect_github_logs_pid" &> /dev/null ); then
  sleep 3
